...
# ERROR_TABLE = [
    # (b"/http-server/src/server.py HTTP/1.1 200\r\nHost: 127.0.0.1:5000\r\n", "405 error: only GET method accepted")
# ]
...
def test_parse_function_raises_value_error_with_405():
    ...
    with pytest.raises(ValueError, match='405 error: only GET method accepted'):
...
def test_parse_function_raises_value_error_with_505():
    """Read function name"""
    from server import parse_request
    with pytest.raises(ValueError, match='505 error: HTTP Request is not version 1.1'):
